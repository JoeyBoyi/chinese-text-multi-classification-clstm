...
def train_cnn_rnn():
	x_, y_, vocabulary, embedding_mat, labels = data.Issue(params["global_path"]+params["input_data"]).parse(w2v_embeddings=True)
	...
	with graph.as_default():
		...
		with sess.as_default():
			...
			optimizer = tf.train.RMSPropOptimizer(params["learning_rate"], decay=0.9)
			grads_and_vars = optimizer.compute_gradients(cnn_rnn.loss)
			train_op = optimizer.apply_gradients(grads_and_vars, global_step=global_step)

			# board_dir = './training_logs_' + timestamp
			# os.makedirs(board_dir)
			# summary_writer = tf.train.SummaryWriter(board_dir, sess.graph)
