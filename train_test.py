...
data, labels, lengths, vocab_processor, label_dict = data_helper.Issue(FLAGS.data_file). \
                                                load_data(sw_path=FLAGS.stop_word_file,
...
# print (vocab_processor.vocabulary_._mapping)  # 字典词汇表，{词：索引}
...
# Save labels for the prediction
with open(outdir + 'labels.json', 'w') as outfile:
    json.dump(list(label_dict.keys()), outfile, indent=4, ensure_ascii=False)

# Simple Cross validation
# TODO use k-fold cross validation
# Split the original dataset into train set and test set
x, x_test, y, y_test, data_lengths, test_lengths = train_test_split(data,
                                                                 labels,
                                                                 lengths,
...
x_train, x_valid, y_train, y_valid, train_lengths, valid_lengths = train_test_split(x,
                                                                                    y,
                                                                                    data_lengths,
                                                                                    test_size=FLAGS.test_size,
                                                                                    random_state=22)
...
with tf.Graph().as_default():
    with tf.Session() as sess:
        ...
        def run_step(input_data, is_training=True, is_test=False):
        ...
        print('\nTest')
        ckpt = tf.train.get_checkpoint_state(os.path.join(outdir, 'model'))
        saver.restore(sess, ckpt.model_checkpoint_path)
        test_step((x_test, y_test, test_lengths))
